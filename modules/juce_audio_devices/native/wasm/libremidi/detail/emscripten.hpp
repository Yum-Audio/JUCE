...
public:
  ...
  {
    return MAIN_THREAD_EM_ASM_INT(return typeof globalThis.__libreMidi_access !== undefined;);
  }

  const int input_count() const noexcept
  {
    return MAIN_THREAD_EM_ASM_INT(return globalThis.__libreMidi_currentInputs.length;);
  }

  const int output_count() const noexcept
  {
    return MAIN_THREAD_EM_ASM_INT(return globalThis.__libreMidi_currentOutputs.length;);
  }

  const void load_current_infos() noexcept
  {
#define get_js_string(variable_to_read, ...) \
  (char*)MAIN_THREAD_EM_ASM_INT({ \
  ...
  return str; \
    ...
    for(int i = 0; i < inputs; i++)
    {
      ...
      const bool connected = MAIN_THREAD_EM_ASM_INT(return globalThis.__libreMidi_currentInputs[$0].state === "connected", i);
    ...
    for(int i = 0; i < outputs; i++)
    {
      ...
      const bool connected = MAIN_THREAD_EM_ASM_INT(return globalThis.__libreMidi_currentOutputs[$0].state === "connected", i);
